...
#define PHALCON_REGISTER_CLASS(ns, class_name, name, methods, flags) \
	{ \
		...
		extname_ ##name## _ce = zend_register_internal_class(&ce); \
		extname_ ##name## _ce->ce_flags |= flags;  \
	}
...
#define PHALCON_REGISTER_CLASS_CREATE_OBJECT(ns, class_name, name, methods, flags) \
	{ \
		...
		extname_ ##name## _object_handlers.offset = XtOffsetOf(phalcon_ ##name## _object, std); \
		extname_ ##name## _object_handlers.free_obj = (zend_object_free_obj_t) phalcon_ ##name## _object_free_handler; \
		INIT_NS_CLASS_ENTRY(ce, #ns, #class_name, methods); \
		extname_ ##name## _ce = zend_register_internal_class(&ce); \
		extname_ ##name## _ce->create_object = phalcon_ ##name## _object_create_handler; \
		extname_ ##name## _ce->ce_flags |= flags;  \
	}
...
#define PHALCON_REGISTER_CLASS_EX(ns, class_name, lcname, parent_ce, methods, flags) \
	{ \
		zend_class_entry ce; \
		INIT_NS_CLASS_ENTRY(ce, #ns, #class_name, methods); \
		extname_ ##lcname## _ce = zend_register_internal_class_ex(&ce, parent_ce); \
		if (!extname_ ##lcname## _ce) { \
...
#define PHALCON_REGISTER_CLASS_CREATE_OBJECT_EX(ns, class_name, lcname, parent_ce, methods, flags) \
	{ \
		...
		extname_ ##lcname## _object_handlers.offset = XtOffsetOf(phalcon_ ##lcname## _object, std); \
		extname_ ##lcname## _object_handlers.free_obj = (zend_object_free_obj_t) phalcon_ ##lcname## _object_free_handler; \
		INIT_NS_CLASS_ENTRY(ce, #ns, #class_name, methods); \
		extname_ ##lcname## _ce = zend_register_internal_class_ex(&ce, parent_ce); \
		if (!extname_ ##lcname## _ce) { \
			fprintf(stderr, "Phalcon Error: Class to extend '%s' was not found when registering class '%s'\n", (parent_ce ? parent_ce->name->val : "(null)"), ZEND_NS_NAME(#ns, #class_name)); \
		...
		extname_ ##lcname## _ce->create_object = phalcon_ ##lcname## _object_create_handler; \
		extname_ ##lcname## _ce->ce_flags |= flags;  \
	}
...
#define PHALCON_REGISTER_INTERFACE(ns, classname, name, methods) \
	{ \
		...
		extname_ ##name## _ce = zend_register_internal_interface(&ce); \
	}
...
#define PHALCON_REGISTER_INTERFACE_EX(ns, classname, lcname, parent_ce, methods) \
	{ \
		zend_class_entry ce; \
		INIT_NS_CLASS_ENTRY(ce, #ns, #classname, methods); \
		extname_ ##lcname## _ce = phalcon_register_internal_interface_ex(&ce, parent_ce); \
		if (!extname_ ##lcname## _ce) { \
